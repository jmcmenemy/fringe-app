...
const OC = {
  ...
  "theSpace":       { bg: "#0EA5E9", glow: "rgba(14,165,233,0.3)" },
  "Zoo":            { bg: "#06B6D4", glow: "rgba(6,182,212,0.3)" },
  "C ARTS":         { bg: "#E11D48", glow: "rgba(225,29,72,0.3)" },
  "Summerhall":     { bg: "#8B5CF6", glow: "rgba(139,92,246,0.3)" },
  "Traverse":       { bg: "#EC4899", glow: "rgba(236,72,153,0.3)" },
  "Heroes":         { bg: "#10B981", glow: "rgba(16,185,129,0.3)" },
  "Greenside":      { bg: "#D97706", glow: "rgba(217,119,6,0.3)" },
  "Free Festival":  { bg: "#059669", glow: "rgba(5,150,105,0.3)" },
  "PBH":            { bg: "#14B8A6", glow: "rgba(20,184,166,0.3)" },
...
function parseDuration(s,e){const sm=timeToMinutes(parseTime(s));const em=timeToMinutes(parseTime(e));if(sm===null||em===null)return"";if(sm===em)return"";let d=em-sm;if(d<0)d+=24*60;const h=Math.floor(d/60);const m=d%60;if(h&&m)return`${h}h${m.toString().padStart(2,"0")}`;if(h)return`${h}h`;return`${m}m`;}
function durationMinutes(show){const s=timeToMinutes(show.start);const e=timeToMinutes(show.end);if(s===null||e===null)return 0;if(s===e)return 0;let d=e-s;if(d<0)d+=24*60;return d;}
function parseCSVToShows(csv){
  ...
  for(let k=1;k<rows.length;k++){
    ...
    const show={name,link:g(row,iLink),price:g(row,iPrice),organiser:g(row,iOrg),
      ...
      notes:g(row,iNotes),address:g(row,iAddr),fullAddress:g(row,iAddr),availability:g(row,iAvail),genres:g(row,iGenre),joCat:g(row,iJoCat),lat:pf(row,iLat),lng:pf(row,iLng)};
...
function importAllData(token){try{var o=JSON.parse(LZString.decompressFromEncodedURIComponent(token));if(!o||typeof o!=="object")return false;Object.keys(o).forEach(function(k){if(k.indexOf("fringe-")===0&&typeof o[k]==="string")localStorage.setItem(k,o[k]);});return true;}catch(e){return false;}}
...
// Hardcoded to Europe/London because all Fringe performance times are local Edinburgh wall-clock times
...
function FringeCalendarInner(){
  ...
  const[addUrl,setAddUrl]=useState("");const[addName,setAddName]=useState("");
  ...
  useEffect(()=>{(async()=>{try{const res=await fetch(SHEET_URL);if(!res.ok)return;const text=await res.text();const{shows,wishlist:wl}=parseCSVToShows(text);if(shows.length>0){setAllShows([...shows,...catAddsRef.current.book]);setWishlist([...wl,...catAddsRef.current.wish]);setDataSource("live");setLastUpdated(new Date());}}catch{}})();},[]);
  ...
  const SH=110;const {START_H,END_H}=useMemo(()=>{let minM=Infinity,maxM=-Infinity;const adj=m=>m==null?null:(m<300?m+1440:m);weekDates.forEach(ds=>(showsByDate[ds]||[]).forEach(s=>{const st=adj(timeToMinutes(s.start));if(st==null)return;let en=adj(timeToMinutes(s.end));if(en==null||en<=st)en=st+60;if(st<minM)minM=st;if(en>maxM)maxM=en;}));if(minM===Infinity)return{START_H:10,END_H:20};const sH=Math.floor((minM-30)/60),eH=Math.ceil((maxM+30)/60);return{START_H:sH,END_H:Math.max(sH+1,eH)};},[weekDates,showsByDate]);const totalSlots=(END_H-START_H);
  ...
  useEffect(()=>{if(view==="calendar"&&gridRef.current){const todayIdx=weekDates.indexOf(todayStr);if(calOrientation==="vertical"){// Scroll vertically to first show
      const first=weekDates.reduce((min,date)=>{(showsByDate[date]||[]).forEach(s=>{let m=timeToMinutes(s.start);if(m!==null){if(m<START_H*60)m+=24*60;if(m<min)min=m;}});return min;},Infinity);if(first<Infinity)gridRef.current.scrollTop=Math.max(0,((first-START_H*60)/60)*SH-20);
      // Scroll horizontally to center today's column
      if(todayIdx>=0){const colW=gridRef.current.querySelector("div")?.offsetWidth||0;const timeColW=48;const dayColW=colW>0?Math.max(78,(colW-timeColW)/weekDates.length):78;const targetLeft=timeColW+todayIdx*dayColW+dayColW/2-gridRef.current.clientWidth/2;gridRef.current.scrollLeft=Math.max(0,targetLeft);}
    }else{// Horizontal: scroll to center today's row
      if(todayIdx>=0){const ROW_H=80;const headerH=40;const targetTop=headerH+todayIdx*ROW_H+ROW_H/2-gridRef.current.clientHeight/2;gridRef.current.scrollTop=Math.max(0,targetTop);}
      // Also scroll horizontally to first show
      const first=weekDates.reduce((min,date)=>{(showsByDate[date]||[]).forEach(s=>{let m=timeToMinutes(s.start);if(m!==null){if(m<START_H*60)m+=24*60;if(m<min)min=m;}});return min;},Infinity);if(first<Infinity){const CW=100;const timeOffset=80;const targetLeft=timeOffset+((first-START_H*60)/60)*CW-gridRef.current.clientWidth/3;gridRef.current.scrollLeft=Math.max(0,targetLeft);}}}},[view,weekIdx,currentMonday,showsByDate,customRange,calOrientation]);
  ...
  const bookingsByDate=useMemo(()=>{const map={};filteredShows.forEach(s=>{if(!showOldBookings){const e=bookingEnd_(s);if(e&&e<now)return;}if(!map[s.date])map[s.date]=[];map[s.date].push(s);});Object.values(map).forEach(arr=>arr.sort((a,b)=>(timeToMinutes(a.start)||0)-(timeToMinutes(b.start)||0)));return map;},[filteredShows,now,showOldBookings]);
  ...
  const refreshData=async()=>{try{const res=await fetch(SHEET_URL);if(!res.ok)return;const text=await res.text();const{shows,wishlist:wl}=parseCSVToShows(text);if(shows.length>0){setAllShows([...shows,...catAddsRef.current.book]);setWishlist([...wl,...catAddsRef.current.wish]);setDataSource("live");setLastUpdated(new Date());}}catch{}};
  ...
  const handleAddShow=async()=>{
    ...
    try{
      const newRec={id:Date.now(),name:addName||"Show from link",organiser:"",venue:"",start:"",end:"",price:"",duration:"",address:"",description:"",link:url,isRecommendation:true};
      await saveRecs([...recommendations,newRec]);
      if(APPS_SCRIPT_URL){try{await fetch(APPS_SCRIPT_URL,{method:"POST",mode:"no-cors",headers:{"Content-Type":"application/json"},body:JSON.stringify({name:newRec.name,link:newRec.link})});}catch{}}
      setAddUrl("");setAddName("");setShowAddModal(false);
    ...
    <div style={{fontFamily:"'Inter',system-ui,-apple-system,sans-serif",maxWidth:960,margin:"0 auto",color:TXT,padding:isMobile?"0 4px 88px":"0 4px",background:BG,minHeight:"100vh"}}>
        ...
        {showFilterMenu&&view!=="proposal"&&view!=="jospicks"&&(
          <div style={{position:"fixed",left:isMobile?0:"auto",right:isMobile?0:16,bottom:isMobile?66:16,zIndex:69,background:BG,borderTop:`1px solid ${CARD_BORDER}`,padding:"12px 8px 14px",overflow:"visible",boxShadow:"0 -8px 30px rgba(0,0,0,0.55)",borderRadius:isMobile?0:16,maxWidth:isMobile?"100%":600}}>
            <div style={{display:"flex",gap:8,flexWrap:"wrap",alignItems:"center",justifyContent:"center"}}>
              ...
              {view!=="funfacts"&&fHas.org&&<MultiDrop up={true} open={openDrop==="org"} onToggle={()=>setOpenDrop(openDrop==="org"?null:"org")} label="Organisers" selected={orgFilter} onSelect={toggleOrg} onClear={()=>setOrgFilter([])} options={organiserChips.map(o=>({value:o,label:o,dot:gc(o).bg}))} maxH={isMobile?200:280}/>}
              {view!=="funfacts"&&fHas.genre&&availableGenres.length>0&&<MultiDrop up={true} open={openDrop==="genre"} onToggle={()=>setOpenDrop(openDrop==="genre"?null:"genre")} label="Genre" selected={genreFilter} onSelect={toggleGenre} onClear={()=>setGenreFilter([])} options={availableGenres.map(gn=>({value:gn,label:gn}))} maxH={isMobile?200:280}/>}
              {people.length>0&&fHas.people&&(
                <MultiDrop up={true} open={openDrop==="people"} onToggle={()=>setOpenDrop(openDrop==="people"?null:"people")} label="People" icon="👥" selected={peopleFilter} onSelect={togglePerson} onClear={()=>setPeopleFilter([])} options={people.map(nm=>({value:nm,label:nm}))} maxH={isMobile?200:280}/>
              )}
              {view!=="funfacts"&&fHas.time&&<MultiDrop up={true} open={openDrop==="time"} onToggle={()=>setOpenDrop(openDrop==="time"?null:"time")} label="Time" selected={timeFilters} onSelect={toggleTime} onClear={()=>setTimeFilters([])} options={[{value:"morning",label:"🌅 Morning"},{value:"afternoon",label:"☀️ Afternoon"},{value:"evening",label:"🌆 Evening"},{value:"late",label:"🌙 Late"}]} maxH={isMobile?200:280}/>}
            </div>
          </div>
        )}
      </div>
      ...
      {view!=="proposal"&&view!=="jospicks"&&(()=>{const sz=48;const def={x:(typeof window!=="undefined"?window.innerWidth:400)-sz-12,y:(typeof window!=="undefined"?window.innerHeight:800)-sz-84};const pos=fabPos||def;return(
        <button
          ...
          style={{position:"fixed",left:pos.x,top:pos.y,zIndex:72,width:sz,height:sz,borderRadius:24,border:"none",background:ACCENT,color:"#fff",display:"flex",alignItems:"center",justifyContent:"center",boxShadow:"0 6px 20px rgba(255,77,106,0.4)",cursor:"grab",touchAction:"none"}}>
          <FilterIcon/>
        </button>);})()}
      {isMobile&&(
        <div style={{position:"fixed",left:0,right:0,bottom:0,zIndex:70,background:"var(--card-solid)",borderTop:`1px solid ${CARD_BORDER}`,display:"flex",gap:8,alignItems:"center",padding:"8px 10px calc(8px + env(safe-area-inset-bottom))"}}>
          <div role="navigation" aria-label="Sections" style={{flex:1,display:"flex",gap:3,justifyContent:"space-evenly",overflowX:"auto"}}>
            {[["calendar","🗓","Calendar"],["map","🗺️","Map"],["list","🎫","Bookings"],["wishlist","💜","Wishlist"],["jospicks","⭐","Browse catalogue"],["proposal","📋","Proposal"],["funfacts","🎉","Stats"]].map(([id,ic,lbl])=>(
              <button key={id} onClick={()=>setView(id)} aria-label={lbl} title={lbl} style={{flex:"0 0 auto",width:46,height:46,display:"inline-flex",alignItems:"center",justifyContent:"center",borderRadius:12,border:`1px solid ${view===id?"#a855f7":CARD_BORDER}`,cursor:"pointer",fontSize:23,lineHeight:1,background:view===id?"rgba(168,85,247,0.25)":"transparent"}}>{ic}</button>
            ))}
          </div>
      ...
      {view==="calendar"&&(
        <div style={{padding:"8px 0"}}>
          <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"6px 8px 10px",position:"sticky",top:52,zIndex:25,background:BG}}>
            <div style={{display:"inline-flex",borderRadius:10,border:`1px solid ${CARD_BORDER}`,overflow:"hidden",marginRight:8}}>{[["vertical",<RowsIcon/>],["horizontal",<ColsIcon/>]].map(([v,ic])=><button key={v} onClick={()=>setCalOrientation(v)} title={v==="vertical"?"Days across, times down":"Times across, days down"} style={{padding:"6px 10px",border:"none",cursor:"pointer",fontSize:14,fontWeight:700,background:calOrientation===v?ACCENT:"transparent",color:calOrientation===v?"#fff":TXT2,display:"flex",alignItems:"center"}}>{ic}</button>)}</div>
            <button onClick={()=>{setPickStart(weekDates[0]);setPickEnd(weekDates[weekDates.length-1]);setDatePickerOpen(o=>!o);}} title="Tap to choose your own dates" style={{fontSize:17,fontWeight:700,color:TXT,background:"none",border:"none",cursor:"pointer",display:"inline-flex",alignItems:"center",justifyContent:"center",gap:6,padding:"4px 8px",borderRadius:10,flex:1}}>{(()=>{const a=new Date(weekDates[0]+"T12:00:00");const b=new Date(weekDates[weekDates.length-1]+"T12:00:00");return weekDates.length===1?`${a.getDate()} ${MONTHS[a.getMonth()]}`:`${a.getDate()} ${MONTHS[a.getMonth()]} – ${b.getDate()} ${MONTHS[b.getMonth()]}`;})()} <span style={{fontSize:11,color:TXT2}}>▾</span></button>
            <div style={{display:"inline-flex",gap:6}}><NavBtn disabled={!customRange&&weekIdx===0} onClick={navPrev}>‹</NavBtn><NavBtn disabled={!customRange&&weekIdx>=weeks.length-1} onClick={navNext}>›</NavBtn></div>
            {datePickerOpen&&(
              <div style={{position:"absolute",top:"100%",left:"50%",transform:"translateX(-50%)",zIndex:60,background:"var(--card-solid)",border:`1px solid ${CARD_BORDER}`,borderRadius:14,padding:14,boxShadow:"0 12px 40px rgba(0,0,0,0.6)",minWidth:isMobile?220:250}}>
                {isMobile?(
                  <>
                    <div style={{fontSize:12,color:TXT2,marginBottom:10,fontWeight:600,textAlign:"center"}}>Pick a day</div>
                    <input type="date" value={pickStart} onChange={e=>{const v=e.target.value;if(v){setCustomRange({start:v,end:v});setDatePickerOpen(false);}}} style={{width:"100%",boxSizing:"border-box",padding:"10px 12px",borderRadius:10,border:`1px solid ${CARD_BORDER}`,background:"var(--card)",color:TXT,fontSize:14,outline:"none",colorScheme:theme==="light"?"light":"dark"}}/>
                    <div style={{display:"flex",gap:6,marginTop:12,justifyContent:"center"}}>
                      <button onClick={()=>{setCustomRange({start:todayStr,end:todayStr});setDatePickerOpen(false);}} style={{padding:"8px 12px",borderRadius:10,border:`1px solid ${CARD_BORDER}`,background:"rgba(168,85,247,0.18)",color:"#C084FC",fontSize:13,fontWeight:700,cursor:"pointer",flex:1}}>Today</button>
                      <button onClick={resetRange} style={{padding:"8px 12px",borderRadius:10,border:`1px solid ${CARD_BORDER}`,background:"transparent",color:TXT2,fontSize:13,fontWeight:700,cursor:"pointer",flex:1}}>Back to weeks</button>
                    </div>
                  </>
                ):(
                  <>
...
{calOrientation==="vertical"&&          <div ref={gridRef} style={{overflow:"auto",maxHeight:isMobile?"calc(100vh - 215px)":"calc(100vh - 120px)",WebkitOverflowScrolling:"touch",position:"relative"}}>
                ...
                {weekDates.map(ds=>{const dayShows=showsByDate[ds]||[];const isToday=ds===todayStr;const timeLineTop=(()=>{if(!isToday)return null;let m=nowMinutes;if(m<START_H*60)m+=24*60;if(m>=END_H*60)return null;return((m-START_H*60)/60)*SH;})();return(
                  <div key={ds} style={{position:"relative",borderLeft:`1px solid ${CARD_BORDER}`,height:totalSlots*SH,background:isToday?"rgba(168,85,247,0.06)":"transparent"}}>
                    ...
                    {dayShows.map((show,si)=>{const c=gc(show.organiser);const top=showTop(show);const height=showHeight(show);return(
                      <div key={si} role="button" tabIndex={0} aria-label={`${show.name}, ${show.venue}, ${formatTime(show.start)}`} onClick={()=>setSelectedShow(show)} onKeyDown={e=>{if(e.key==="Enter"||e.key===" "){e.preventDefault();setSelectedShow(show);}}} style={{
          ...
          {calOrientation==="horizontal"&&(()=>{const CW=100;const bodyW=80+totalSlots*CW;return <div ref={gridRef} style={{overflow:"auto",maxHeight:isMobile?"calc(100vh - 215px)":"calc(100vh - 120px)",WebkitOverflowScrolling:"touch"}}>
            <div style={{width:bodyW,position:"relative"}}>
              <div style={{display:"flex",position:"sticky",top:0,background:BG,zIndex:20,borderBottom:`1px solid ${CARD_BORDER}`}}>
                <div style={{width:80,flexShrink:0,position:"sticky",left:0,zIndex:21,background:BG}}/>
                {Array.from({length:totalSlots},(_,i)=>{const mins=START_H*60+i*60;const dm=mins>=24*60?mins-24*60:mins;return <div key={i} style={{width:CW,flexShrink:0,textAlign:"center",padding:"8px 0",borderLeft:`1px solid ${CARD_BORDER}`}}><div style={{fontSize:12,fontWeight:700,color:TXT3}}>{formatHour(dm)}</div></div>;})}
              </div>
              {weekDates.map(ds=>{const d=new Date(ds+"T12:00:00");const dayShows=showsByDate[ds]||[];const isToday=ds===todayStr;const has=dayShows.length>0;const ROW_H=80;return(
                <div key={ds} style={{display:"flex",borderBottom:`1px solid ${CARD_BORDER}`,background:isToday?"rgba(168,85,247,0.06)":"transparent",position:"relative",height:ROW_H}}>
                  <div style={{width:80,flexShrink:0,position:"sticky",left:0,zIndex:15,background:isToday?"rgba(168,85,247,0.1)":BG,padding:"8px 6px",display:"flex",flexDirection:"column",justifyContent:"center",borderRight:`1px solid ${CARD_BORDER}`}}>
                    <div style={{fontSize:11,fontWeight:700,color:isToday?"#C084FC":TXT3,textTransform:"uppercase",letterSpacing:0.5}}>{DAY_NAMES_SHORT[(d.getDay()+6)%7]}</div>
                    <div style={{fontSize:18,fontWeight:800,color:isToday?"#C084FC":has?TXT:TXT3}}>{d.getDate()}</div>
                  </div>
                  <div style={{position:"relative",flex:1,minHeight:ROW_H}}>
                    {Array.from({length:totalSlots},(_,i)=><div key={i} style={{position:"absolute",left:i*CW,top:0,bottom:0,width:1,background:i%2===0?CARD_BORDER:"rgba(128,128,128,0.12)"}}/>)}
                    {dayShows.map((show,si)=>{const col=gc(show.organiser);let sm=timeToMinutes(show.start);if(sm===null)return null;if(sm<START_H*60)sm+=24*60;const px=((sm-START_H*60)/60)*CW;const dur=durationMinutes(show)||60;const w=Math.max(CW*0.8,(dur/60)*CW-4);return(
                      <div key={si} role="button" tabIndex={0} aria-label={`${show.name}, ${formatTime(show.start)}`} onClick={()=>setSelectedShow(show)} onKeyDown={e=>{if(e.key==="Enter"||e.key===" "){e.preventDefault();setSelectedShow(show);}}} style={{position:"absolute",left:px,width:w,top:4,bottom:4,background:col.bg,color:"#fff",borderRadius:8,padding:"4px 8px",cursor:"pointer",overflow:"hidden",zIndex:3,fontSize:12,lineHeight:1.25,boxShadow:`0 2px 6px ${col.glow}`,border:!show.booked?"2px dashed rgba(255,255,255,0.4)":"none",opacity:show.booked?1:0.8,display:"flex",flexDirection:"column",justifyContent:"center"}}>
                        <div style={{fontWeight:700,lineHeight:1.2,overflow:"hidden",display:"-webkit-box",WebkitLineClamp:2,WebkitBoxOrient:"vertical"}}>{show.name}</div>
                        <div style={{opacity:0.85,fontSize:11}}>{formatTime(show.start)}</div>
                      </div>);})}
                  </div>
                </div>);})}
            </div>
      ...
      {view==="list"&&(
        <div style={{padding:"8px 0"}}>
          <div style={{display:"flex",alignItems:"center",gap:8,justifyContent:"center",padding:"0 12px 12px"}}>
            ...
            <button onClick={()=>setShowOldBookings(v=>!v)} title={showOldBookings?"Hide past shows":"Show past shows"} aria-label={showOldBookings?"Hide past shows":"Show past shows"} style={{padding:"8px 10px",borderRadius:12,border:`1px solid ${showOldBookings?"#a855f7":CARD_BORDER}`,background:showOldBookings?"rgba(168,85,247,0.2)":"transparent",color:showOldBookings?"#C084FC":TXT3,fontSize:15,cursor:"pointer",display:"inline-flex",alignItems:"center",lineHeight:1}}>{showOldBookings?"👁":"👁‍🗨"}</button>
      ...
      {showAddModal&&(
        <div onClick={()=>!addLoading&&setShowAddModal(false)} style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.7)",backdropFilter:"blur(4px)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:1000,padding:16}}>
          <div onClick={e=>e.stopPropagation()} style={{background:"var(--card-solid)",border:`1px solid ${CARD_BORDER}`,borderRadius:20,padding:28,maxWidth:440,width:"100%",boxShadow:"0 24px 80px rgba(0,0,0,0.6)"}}>
            ...
            <p style={{fontSize:14,color:TXT2,margin:"0 0 16px"}}>Add a show name and link from any Edinburgh Fringe venue website</p>
            <input type="text" placeholder="Show name" value={addName} onChange={e=>setAddName(e.target.value)} disabled={addLoading}
              style={{width:"100%",padding:"12px 14px",borderRadius:12,border:`1px solid ${CARD_BORDER}`,fontSize:14,outline:"none",color:TXT,background:"var(--card)",boxSizing:"border-box",marginBottom:10}}/>
      ...
      {view==="jospicks"&&(
        <div style={{padding:"14px 12px 50px",maxWidth:1000,margin:"0 auto"}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",gap:8,margin:"0 0 12px",flexWrap:"wrap"}}>
            <p style={{fontSize:13,color:TXT2,margin:0}}>Browse the full Fringe programme{allCatalog?(" · "+allCatalog.length.toLocaleString()+" shows"):""}.</p>
            <div style={{display:"inline-flex",borderRadius:10,border:`1px solid ${CARD_BORDER}`,overflow:"hidden"}}>{[["cards","▦ Cards"],["tiles","⊞ Tiles"],["table","▤ Table"]].map(([v,lbl])=>(<button key={v} onClick={()=>setCatView(v)} style={{padding:"7px 12px",border:"none",cursor:"pointer",fontSize:12,fontWeight:800,background:catView===v?ACCENT:"transparent",color:catView===v?"#fff":TXT2}}>{isMobile?lbl.split(" ")[0]:lbl}</button>))}</div>
          </div>
          <input value={searchQuery} onChange={e=>setSearchQuery(e.target.value)} placeholder="🔎 Search shows, artists, venues…" style={{width:"100%",boxSizing:"border-box",padding:"11px 14px",borderRadius:12,border:`1px solid ${CARD_BORDER}`,background:"var(--card)",color:TXT,fontSize:14,outline:"none",marginBottom:12}}/>
          {catState==="loading"?<div style={{textAlign:"center",color:TXT3,fontSize:14,padding:"40px 10px"}}>Loading the full catalogue…</div>:catState==="error"?<div style={{textAlign:"center",color:TXT3,fontSize:14,padding:"40px 10px"}}>Couldn’t load the catalogue — check the All-tab CSV link.</div>:(()=>{const base=allCatalog||[];let filtered=catFiltered;if(catView==="table"){const{key,dir}=catSort;filtered=[...filtered].sort((a,b)=>{let va,vb;if(key==="price"){va=poundsOf(a.price);vb=poundsOf(b.price);}else if(key==="start"){va=timeToMinutes(a.start)==null?1e9:timeToMinutes(a.start);vb=timeToMinutes(b.start)==null?1e9:timeToMinutes(b.start);}else if(key==="duration"){va=durationMinutes(a);vb=durationMinutes(b);}else{va=String(a[key]||"").toLowerCase();vb=String(b[key]||"").toLowerCase();}return va<vb?-dir:va>vb?dir:0;});}const capped=filtered.slice(0,120);const actBtns=(s,iw,ib)=>(<span style={{display:"inline-flex",gap:5,flexShrink:0}}><button onClick={e=>{e.stopPropagation();catAddWish(s);}} title={iw?"Remove from wishlist":"Add to wishlist"} style={{width:30,height:30,borderRadius:8,border:`1px solid ${iw?"#a855f7":CARD_BORDER}`,background:iw?"rgba(168,85,247,0.2)":"transparent",color:iw?"#C084FC":TXT3,fontSize:14,cursor:"pointer"}}>{iw?"💜":"♡"}</button><button onClick={e=>{e.stopPropagation();setBookingShow(s);}} title={ib?"Booked":"Mark as booked"} style={{width:30,height:30,borderRadius:8,border:`1px solid ${ib?"#34D399":CARD_BORDER}`,background:ib?"rgba(52,211,153,0.2)":"transparent",color:ib?"#34D399":TXT3,fontSize:13,cursor:"pointer"}}>🎫</button></span>);return (<>
            {catView==="cards"&&<div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill, minmax(300px, 1fr))",gridAutoRows:"252px",gap:12}}>{capped.map(s=>{const iw=wishlist.some(w=>reviewKey(w)===reviewKey(s));const ib=allShows.some(w=>w.booked&&reviewKey(w)===reviewKey(s));return <JoCard key={s.name+"|"+s.venue} show={s} readOnly onOpen={()=>setSelectedShow(s)} onWish={()=>catAddWish(s)} onBook={()=>setBookingShow(s)} inWish={iw} inBook={ib}/>;})}</div>}
            {catView==="tiles"&&<div style={{display:"flex",flexDirection:"column",gap:8}}>{capped.map(s=>{const iw=wishlist.some(w=>reviewKey(w)===reviewKey(s));const ib=allShows.some(w=>w.booked&&reviewKey(w)===reviewKey(s));return <div key={s.name+"|"+s.venue} style={{display:"flex",alignItems:"center",gap:10,padding:"10px 12px",borderRadius:10,background:"var(--card-solid)",border:`1px solid ${CARD_BORDER}`,borderLeft:`4px solid ${gc2(s.organiser).bg}`}}><div onClick={()=>setSelectedShow(s)} style={{flex:1,minWidth:0,cursor:"pointer"}}><div style={{fontSize:15,fontWeight:800,color:TXT,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{s.start?formatTime(s.start)+" · ":""}{s.name}</div><div style={{fontSize:12,color:TXT3,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>📍 {s.venueCode?"#"+s.venueCode+" ":""}{s.venue}{s.price?" · "+s.price:""}</div></div>{actBtns(s,iw,ib)}</div>;})}</div>}
            {catView==="table"&&<div style={{overflowX:"auto"}}><table style={{width:"100%",borderCollapse:"collapse",fontSize:13}}><thead><tr>{[["name","Title"],["start","Start"],["duration","Duration"],["venue","Venue"],["price","Price"],["genres","Genre"]].map(([k,lbl])=>(<th key={k} onClick={()=>setCatSort(pr=>({key:k,dir:pr.key===k?-pr.dir:1}))} style={{textAlign:"left",padding:"8px 10px",borderBottom:`1px solid ${CARD_BORDER}`,color:catSort.key===k?TXT:TXT2,cursor:"pointer",whiteSpace:"nowrap",fontWeight:800}}>{lbl}{catSort.key===k?(catSort.dir>0?" ▲":" ▼"):""}</th>))}<th style={{borderBottom:`1px solid ${CARD_BORDER}`}}></th></tr></thead><tbody>{capped.map(s=>{const iw=wishlist.some(w=>reviewKey(w)===reviewKey(s));const ib=allShows.some(w=>w.booked&&reviewKey(w)===reviewKey(s));return <tr key={s.name+"|"+s.venue} style={{borderBottom:`1px solid ${CARD_BORDER}`}}><td onClick={()=>setSelectedShow(s)} style={{padding:"8px 10px",fontWeight:700,color:TXT,cursor:"pointer",maxWidth:220,overflow:"hidden",textOverflow:"ellipsis"}}>{s.name}</td><td style={{padding:"8px 10px",color:TXT2,whiteSpace:"nowrap"}}>{s.start?formatTime(s.start):"—"}</td><td style={{padding:"8px 10px",color:TXT2,whiteSpace:"nowrap"}}>{s.duration||"—"}</td><td style={{padding:"8px 10px",color:TXT2}}>{s.venueCode?"#"+s.venueCode+" ":""}{s.venue}</td><td style={{padding:"8px 10px",color:TXT,whiteSpace:"nowrap"}}>{s.price||"—"}</td><td style={{padding:"8px 10px",color:TXT3}}>{s.genres||s.genre||"—"}</td><td style={{padding:"8px 10px"}}>{actBtns(s,iw,ib)}</td></tr>;})}</tbody></table></div>}
          ...
          <button onClick={()=>setCatFilterOpen(true)} aria-label="Filters" title="Filters" style={{position:"fixed",right:16,bottom:isMobile?92:26,zIndex:500,width:52,height:52,borderRadius:26,border:"none",cursor:"pointer",background:ACCENT,color:"#fff",boxShadow:"0 6px 20px rgba(255,77,106,0.4)",display:"flex",alignItems:"center",justifyContent:"center"}}>
          ...
          {catFilterOpen&&(
            <div onClick={()=>setCatFilterOpen(false)} style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.65)",zIndex:1300,display:"flex",alignItems:isMobile?"flex-end":"flex-start",justifyContent:"center",padding:isMobile?"0":"40px 12px",overflowY:isMobile?"hidden":"auto"}}>
              <div onClick={e=>e.stopPropagation()} style={{background:"var(--card-solid)",border:isMobile?"none":`1px solid ${CARD_BORDER}`,borderTop:isMobile?`1px solid ${CARD_BORDER}`:"none",borderRadius:isMobile?"16px 16px 0 0":16,maxWidth:760,width:"100%",padding:isMobile?"16px 16px calc(24px + env(safe-area-inset-bottom))":"16px 16px 24px",maxHeight:isMobile?"92vh":"85vh",overflowY:"auto",boxSizing:"border-box",WebkitOverflowScrolling:"touch"}}>
                ...
                <div style={{marginBottom:10}}><MultiDrop open={catODrop==="venue"} onToggle={()=>setCatODrop(catODrop==="venue"?null:"venue")} label="Venue (name, # or postcode)" icon="📍" selected={catVenue} onSelect={toggleCatVenue} onClear={()=>setCatVenue([])} options={catVenueOpts}/></div>
                <div style={{display:"flex",gap:8,marginBottom:10,flexWrap:"wrap"}}><MultiDrop open={catODrop==="genre"} onToggle={()=>setCatODrop(catODrop==="genre"?null:"genre")} label="All genres" icon="🎭" selected={catGenre} onSelect={toggleCatGenre} onClear={()=>setCatGenre([])} options={catGenreOpts.map(g=>({value:g,label:g}))}/><select value={catExp} onChange={e=>setCatExp(e.target.value)} aria-label="Experience level" style={{...catSelStyle,width:"auto",flex:"0 1 auto"}}><option value="">Experience level</option>{catExpOpts.map(x=><option key={x} value={x}>{x}</option>)}</select></div>
                ...
                <div style={{display:"grid",gridTemplateColumns:isMobile?"1fr":"1fr 1fr",gap:8,marginBottom:12}}>
                  ...
                  {!isMobile&&<div style={{display:"flex",flexDirection:"column",gap:4}}><span style={{fontSize:11,color:TXT3,fontWeight:700}}>📅 Runs between</span><div style={{display:"flex",gap:6}}><input type="date" value={catRunFrom} onChange={e=>setCatRunFrom(e.target.value)} style={catSelStyle}/><input type="date" value={catRunTo} onChange={e=>setCatRunTo(e.target.value)} style={catSelStyle}/></div></div>}
                </div>
                ...
                <div style={{background:"rgba(255,255,255,0.03)",border:`1px solid ${CARD_BORDER}`,borderRadius:10,padding:"10px 12px",marginBottom:14}}>
                  <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:8}}>
                    <span style={{fontSize:12,fontWeight:800,color:TXT,whiteSpace:"nowrap"}}>💷 Price</span>
                    <span style={{fontSize:12,color:TXT2}}>£</span>
                    <input type="number" min={0} max={catPriceMax} value={catPMin} onChange={e=>{const v=Math.max(0,Math.min(Number(e.target.value)||0,catPMax==null?catPriceMax:catPMax));setCatPMin(v);}} style={{width:52,padding:"5px 6px",borderRadius:8,border:`1px solid ${CARD_BORDER}`,background:"var(--card)",color:TXT,fontSize:13,textAlign:"center",outline:"none",boxSizing:"border-box"}}/>
                    <span style={{fontSize:12,color:TXT3}}>to £</span>
                    <input type="number" min={0} max={catPriceMax} value={catPMax==null?catPriceMax:catPMax} onChange={e=>{const v=Number(e.target.value)||0;setCatPMax(v>=catPriceMax?null:Math.max(v,catPMin));}} style={{width:52,padding:"5px 6px",borderRadius:8,border:`1px solid ${CARD_BORDER}`,background:"var(--card)",color:TXT,fontSize:13,textAlign:"center",outline:"none",boxSizing:"border-box"}}/>
                    {catPMax==null&&<span style={{fontSize:11,color:TXT3}}>+</span>}
                  </div>
                  <div style={{position:"relative",height:28,display:"flex",alignItems:"center"}}>
                    <input type="range" min={0} max={catPriceMax} value={catPMin} onChange={e=>{const v=Number(e.target.value);setCatPMin(Math.min(v,catPMax==null?catPriceMax:catPMax));}} style={{position:"absolute",width:"100%",accentColor:"#A855F7",pointerEvents:"auto",zIndex:catPMin>((catPMax==null?catPriceMax:catPMax)/2)?2:1}}/>
                    <input type="range" min={0} max={catPriceMax} value={catPMax==null?catPriceMax:catPMax} onChange={e=>{const v=Number(e.target.value);setCatPMax(v>=catPriceMax?null:Math.max(v,catPMin));}} style={{position:"absolute",width:"100%",accentColor:"#A855F7",pointerEvents:"auto",zIndex:catPMin>((catPMax==null?catPriceMax:catPMax)/2)?1:2}}/>
                  </div>
                </div>
...
function BookingDialog({show,onClose,onConfirm}){
  ...
  const can=!!date&&!!time;
  return (<div onClick={onClose} style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.7)",zIndex:1400,display:"flex",alignItems:"center",justifyContent:"center",padding:16}}>
    <div onClick={e=>e.stopPropagation()} style={{background:"var(--card-solid)",border:`1px solid ${CARD_BORDER}`,borderRadius:16,maxWidth:380,width:"100%",padding:22,boxSizing:"border-box"}}>
      ...
      <input type="date" value={date} onChange={e=>setDate(e.target.value)} style={{width:"100%",boxSizing:"border-box",padding:"9px 12px",borderRadius:10,border:`1px solid ${CARD_BORDER}`,background:"var(--card)",color:TXT,fontSize:14,outline:"none",colorScheme:(typeof document!=="undefined"&&document.documentElement.getAttribute("data-theme")==="light")?"light":"dark"}} />
      <div style={{fontSize:12,color:TXT3,fontWeight:700,margin:"12px 0 4px"}}>Start time {needTime?"*":""}</div>
      <input type="time" value={time} onChange={e=>setTime(e.target.value)} style={{width:"100%",boxSizing:"border-box",padding:"9px 12px",borderRadius:10,border:`1px solid ${CARD_BORDER}`,background:"var(--card)",color:TXT,fontSize:14,outline:"none",colorScheme:(typeof document!=="undefined"&&document.documentElement.getAttribute("data-theme")==="light")?"light":"dark"}} />
      ...
      <div style={{display:"flex",gap:8,justifyContent:"flex-end",marginTop:18}}>
        ...
        <button onClick={()=>{if(can)onConfirm(date,time);}} disabled={!can} style={{padding:"9px 16px",borderRadius:10,border:"none",background:ACCENT,color:"#fff",fontSize:14,fontWeight:800,cursor:can?"pointer":"not-allowed",opacity:can?1:0.5}}>🎫 Confirm</button>
      </div>
    </div>
  </div>);
}
    ...
    <div style={{display:"flex",background:"var(--card)",borderRadius:12,border:`1px solid ${CARD_BORDER}`,overflowY:"auto",overflowX:"hidden",maxHeight:"min(1290px, calc(100vh - 170px))"}}>
      ...
      <div style={{flex:1,position:"relative",height:gh,borderLeft:`1px solid ${CARD_BORDER}`}}>
        ...
        {items.map((it,k)=>{const top=(it._s-rangeTop)/60*HOUR;const bh=Math.max(50,(it._e-it._s)/60*HOUR-2);const w=100/nLanes;const col=gc2(it.organiser).bg;const proposed=!it.booked;return(
          <div key={k} title={it.fullAddress||it.address||it.venue||""} style={{position:"absolute",top,height:bh,left:`calc(${it._lane*w}% + 3px)`,width:`calc(${w}% - 6px)`,background:col,opacity:proposed?0.85:1,borderRadius:8,padding:"5px 8px",overflow:"hidden",color:"#fff",boxSizing:"border-box",zIndex:2,display:"flex",alignItems:"flex-start",justifyContent:"space-between",gap:6,boxShadow:it._ov?"0 0 0 2px #EF4444":"none",border:proposed?"2px dashed rgba(255,255,255,0.5)":"none"}}>
...
function gc2(org){if(!org)return{bg:"#64748B",glow:"rgba(100,116,139,0.3)"};if(OC[org])return OC[org];const lo=org.toLowerCase();for(const k of Object.keys(OC)){if(lo.indexOf(k.toLowerCase())>=0)return OC[k];}return{bg:"#64748B",glow:"rgba(100,116,139,0.3)"}}

function MultiDrop({open,onToggle,label,icon,options,selected,onSelect,onClear,accent="#93C5FD",up,maxH}){
  const n=selected.length;
  const [q,setQ]=useState("");
  const filtered=q.trim()?options.filter(opt=>opt.label.toLowerCase().includes(q.trim().toLowerCase())):options;
  // Reset search when dropdown closes
  const prevOpen=useRef(open);
  useEffect(()=>{if(!open&&prevOpen.current)setQ("");prevOpen.current=open;},[open]);
  return (
    <div style={{position:"relative"}}>
      <button onClick={onToggle} aria-expanded={open} aria-haspopup="listbox" style={{padding:"7px 14px",borderRadius:20,border:`1px solid ${CARD_BORDER}`,fontSize:13,fontWeight:700,cursor:"pointer",background:n?accent+"22":"rgba(255,255,255,0.06)",color:n?accent:TXT,display:"inline-flex",alignItems:"center",gap:6}}>{icon&&<span>{icon}</span>}{n?label+" ("+n+")":label} ▾</button>
      {open&&(<div style={{position:"absolute",...(up?{bottom:"calc(100% + 6px)"}:{top:"calc(100% + 6px)"}),left:0,zIndex:60,background:"var(--card-solid)",border:`1px solid ${CARD_BORDER}`,borderRadius:12,padding:6,minWidth:240,maxHeight:maxH||320,display:"flex",flexDirection:"column",boxShadow:"0 10px 30px rgba(0,0,0,0.6)"}}>
        <input type="text" value={q} onChange={e=>setQ(e.target.value)} placeholder={"Search "+label.toLowerCase()+"…"} autoFocus style={{padding:"7px 10px",borderRadius:8,border:`1px solid ${CARD_BORDER}`,background:"var(--card)",color:TXT,fontSize:12,outline:"none",marginBottom:4,boxSizing:"border-box",width:"100%"}}/>
        {n>0&&<div onClick={onClear} style={{fontSize:12,color:accent,fontWeight:700,padding:"5px 8px",cursor:"pointer",flexShrink:0}}>Clear all ({n})</div>}
        <div style={{overflowY:"auto",flex:1}}>
        {filtered.length===0&&<div style={{fontSize:12,color:TXT3,padding:"7px 8px",whiteSpace:"nowrap"}}>{options.length?"No matches":"Nothing to filter yet"}</div>}
        {filtered.map(opt=>(<label key={opt.value} style={{display:"flex",alignItems:"center",gap:8,padding:"7px 8px",cursor:"pointer",fontSize:13,color:TXT,borderRadius:8,background:selected.includes(opt.value)?accent+"26":"transparent"}}><input type="checkbox" checked={selected.includes(opt.value)} onChange={()=>onSelect(opt.value)} style={{accentColor:accent}}/>{opt.dot&&<span style={{width:9,height:9,borderRadius:3,background:opt.dot,display:"inline-block",flexShrink:0}}/>}{opt.label}</label>))}
        </div>
      </div>)}
    </div>
  );
