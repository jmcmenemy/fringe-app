...
function parseCSVToShows(csv){
  ...
  for(let k=1;k<rows.length;k++){
    ...
    const show={name,link:g(row,iLink),price:g(row,iPrice),organiser:g(row,iOrg),
      ...
      notes:g(row,iNotes),address:g(row,iAddr),fullAddress:(g(row,colAny("full address","fulladdress"))||g(row,iAddr)),availability:g(row,iAvail),genres:g(row,iGenre),joCat:g(row,iJoCat),lat:pf(row,iLat),lng:pf(row,iLng)};
...
function matchesSearch(show,q,overrideAtt){if(!q)return true;const l=q.toLowerCase();return[show.name,show.venue,show.start,show.end,overrideAtt!=null?overrideAtt:show.attendees,show.organiser,show.address,show.price].filter(Boolean).some(f=>f.toLowerCase().includes(l));}
...
function FringeCalendarInner(){
  ...
  const removedRef=useRef(()=>{try{return JSON.parse(localStorage.getItem("fringe-removed")||"[]");}catch{return [];}});
  ...
  const[attNewInput,setAttNewInput]=useState("");
  ...
  const inPeople=s=>{if(!peopleFilter.length)return true;const att=(getAttendees(s)||"").split(",").map(x=>x.trim());return peopleFilter.some(pp=>att.includes(pp));};
  ...
  const filteredShows=useMemo(()=>allShows.filter(s=>s.date).filter(s=>{if(!inStatus(s))return false;if(!inOrg(s))return false;if(!inTime(s))return false;if(!inPeople(s))return false;if(!inGenre(s))return false;if(searchQuery.trim()&&!matchesSearch(s,searchQuery,getAttendees(s)))return false;return true;}),[allShows,statusFilter,orgFilter,timeFilters,peopleFilter,genreFilter,searchQuery,attendeeOverrides]);
  const weeks=useMemo(()=>getWeeks(filteredShows),[filteredShows]);
  useEffect(()=>{if(weekIdx<0&&weeks.length>0){const today=dateToStr(new Date());const todayMon=dateToStr(getMonday(today));const idx=weeks.indexOf(todayMon);setWeekIdx(idx>=0?idx:Math.max(0,weeks.length-1));return;}if(weekIdx>=weeks.length)setWeekIdx(Math.max(0,weeks.length-1));},[weeks,weekIdx]);
          ...
          <div style={{position:"fixed",left:isMobile?0:"auto",right:isMobile?0:16,bottom:isMobile?66:16,zIndex:69,background:BG,borderTop:`1px solid ${CARD_BORDER}`,padding:"12px 8px 14px",overflow:"visible",boxShadow:"0 -8px 30px rgba(0,0,0,0.55)",borderRadius:isMobile?0:16,maxWidth:isMobile?"100%":600}}>
            <div style={{display:"flex",gap:8,flexWrap:"wrap",alignItems:"center",justifyContent:"center"}}>
              ...
              {people.length>0&&fHas.people&&(
                <MultiDrop up={true} open={openDrop==="people"} onToggle={()=>setOpenDrop(openDrop==="people"?null:"people")} label="People" icon="👥" selected={peopleFilter} onSelect={togglePerson} onClear={()=>setPeopleFilter([])} options={people.map(nm=>({value:nm,label:nm}))}/>
              )}
              {view!=="funfacts"&&fHas.time&&<MultiDrop up={true} open={openDrop==="time"} onToggle={()=>setOpenDrop(openDrop==="time"?null:"time")} label="Time" selected={timeFilters} onSelect={toggleTime} onClear={()=>setTimeFilters([])} options={[{value:"morning",label:"🌅 Morning"},{value:"afternoon",label:"☀️ Afternoon"},{value:"evening",label:"🌆 Evening"},{value:"late",label:"🌙 Late"}]}/>}
            </div>
          </div>
        )}
      </div>
      ...
      {view==="calendar"&&(
        <div style={{padding:"8px 0"}}>
          <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"6px 8px 10px",position:"sticky",top:52,zIndex:25,background:BG}}>
            <div style={{display:"inline-flex",borderRadius:10,border:`1px solid ${CARD_BORDER}`,overflow:"hidden",marginRight:8}}>{[["vertical","↕"],["horizontal","↔"]].map(([v,ic])=><button key={v} onClick={()=>setCalOrientation(v)} title={v==="vertical"?"Days across, times down":"Times across, days down"} style={{padding:"6px 10px",border:"none",cursor:"pointer",fontSize:14,fontWeight:700,background:calOrientation===v?ACCENT:"transparent",color:calOrientation===v?"#fff":TXT2}}>{ic}</button>)}</div>
...
{calOrientation==="vertical"&&          <div ref={gridRef} style={{overflow:"auto",maxHeight:isMobile?"calc(100vh - 215px)":"calc(100vh - 120px)",WebkitOverflowScrolling:"touch",position:"relative"}}>
            ...
            <div style={{width:bodyW,position:"relative"}}>
              ...
              {weekDates.map(ds=>{const d=new Date(ds+"T12:00:00");const dayShows=showsByDate[ds]||[];const isToday=ds===todayStr;const has=dayShows.length>0;const ROW_H=80;return(
                <div key={ds} style={{display:"flex",borderBottom:`1px solid ${CARD_BORDER}`,background:isToday?"rgba(168,85,247,0.06)":"transparent",position:"relative",height:ROW_H}}>
                  <div style={{width:80,flexShrink:0,position:"sticky",left:0,zIndex:10,background:isToday?"rgba(168,85,247,0.1)":BG,padding:"8px 6px",display:"flex",flexDirection:"column",justifyContent:"center",borderRight:`1px solid ${CARD_BORDER}`}}>
      ...
      {view==="wishlist"&&(
        <div style={{padding:"16px 8px"}}>
          ...
          <div style={{display:isMobile?"flex":"grid",flexDirection:"column",gridTemplateColumns:isMobile?undefined:`repeat(${wishCols},minmax(0,1fr))`,gap:isMobile?6:12,alignItems:"start"}}>
            {filteredWishlist.map((show,i)=><ShowCard key={i} show={show} onClick={()=>setSelectedShow(show)} review={reviews[reviewKey(show)]} onRate={v=>setReview(show,v)} tags={tagMap[reviewKey(show)]||[]} onAddTag={t=>addTag(show,t)} onRemoveTag={t=>removeTag(show,t)} wishlist interest={interests[reviewKey(show)]} onInterest={v=>setInterest(show,v)} attendees={getAttendees(show)}/>)}
          </div>
        </div>
      )}
      ...
      {addBookingOpen&&(
        <div onClick={()=>{setAddBookingOpen(false);setAddBookingQuery("");}} onKeyDown={e=>{if(e.key==="Escape"){setAddBookingOpen(false);setAddBookingQuery("");}}} tabIndex={-1} style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.7)",backdropFilter:"blur(4px)",display:"flex",alignItems:"flex-start",justifyContent:"center",zIndex:1500,padding:"60px 16px",overflowY:"auto"}}>
          <div onClick={e=>e.stopPropagation()} style={{background:"var(--card-solid)",border:`1px solid ${CARD_BORDER}`,borderRadius:20,padding:24,maxWidth:520,width:"100%",boxSizing:"border-box",boxShadow:"0 24px 80px rgba(0,0,0,0.6)"}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}>
              <div style={{fontSize:20,fontWeight:800,color:TXT}}>Add Booking</div>
              <button onClick={()=>{setAddBookingOpen(false);setAddBookingQuery("");}} style={{background:"none",border:"none",color:TXT2,fontSize:20,cursor:"pointer"}}>✕</button>
            </div>
            <input type="text" autoFocus placeholder="Search by name, venue, organiser, artist..." value={addBookingQuery} onChange={e=>setAddBookingQuery(e.target.value)} style={{width:"100%",padding:"11px 14px",borderRadius:12,border:`1px solid ${CARD_BORDER}`,background:"var(--card)",color:TXT,fontSize:14,outline:"none",boxSizing:"border-box",marginBottom:10}}/>
            {catState==="loading"&&<div style={{textAlign:"center",color:TXT3,fontSize:13,padding:"16px"}}>Loading catalogue...</div>}
            {catState==="error"&&<div style={{textAlign:"center",color:TXT3,fontSize:13,padding:"16px"}}>Could not load catalogue.</div>}
            {allCatalog&&(()=>{const q=addBookingQuery.trim().toLowerCase();const results=q?(allCatalog||[]).filter(s=>[s.name,s.venue,s.organiser,s.artist].filter(Boolean).some(f=>f.toLowerCase().includes(q))).slice(0,8):[];return results.length?(<div style={{maxHeight:420,overflowY:"auto",border:`1px solid ${CARD_BORDER}`,borderRadius:12}}>
              {results.map((s,i)=>{const c=gc2(s.organiser);return(
                <div key={i} style={{display:"flex",alignItems:"center",gap:10,padding:"10px 14px",borderBottom:`1px solid ${CARD_BORDER}`}}>
                  <div style={{flex:1,minWidth:0}}>
                    <div style={{fontSize:14,fontWeight:700,color:TXT,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{s.name}</div>
                    <div style={{fontSize:12,color:TXT2,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{s.venue}{s.start?" · "+formatTime(s.start):""}{s.price?" · "+s.price:""}</div>
                    {s.organiser&&<div style={{fontSize:11,color:c.bg,fontWeight:700,marginTop:2}}>{s.organiser}</div>}
                  </div>
                  <button onClick={()=>{setBookingShow(s);setAddBookingOpen(false);setAddBookingQuery("");}} style={{padding:"7px 14px",borderRadius:10,border:"none",background:ACCENT,color:"#fff",fontSize:12,fontWeight:800,cursor:"pointer",flexShrink:0}}>Book</button>
                </div>);})}
            </div>):(q?<div style={{textAlign:"center",color:TXT3,fontSize:13,padding:"16px"}}>No matches. Try a different search.</div>:<div style={{textAlign:"center",color:TXT3,fontSize:13,padding:"16px"}}>Start typing to search the catalogue.</div>);})()}
          </div>
        </div>
      )}
        ...
        <div onClick={()=>setSelectedShow(null)} onKeyDown={e=>{if(e.key==="Escape")setSelectedShow(null);}} style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.7)",backdropFilter:"blur(4px)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:1000,padding:16}}>
          <div ref={modalRef} role="dialog" aria-modal="true" aria-label={selectedShow.name} onClick={e=>e.stopPropagation()} onTouchStart={e=>{dragStart.current=(modalRef.current&&modalRef.current.scrollTop<=0)?e.touches[0].clientY:null;}} onTouchMove={e=>{if(dragStart.current!=null){const dy=e.touches[0].clientY-dragStart.current;setDragY(dy>0?dy:0);}}} onTouchEnd={()=>{if(dragY>110)setSelectedShow(null);setDragY(0);dragStart.current=null;}} style={{background:"var(--card-solid)",border:`1px solid ${CARD_BORDER}`,borderRadius:20,padding:28,maxWidth:420,width:"100%",boxShadow:"0 24px 80px rgba(0,0,0,0.6)",boxSizing:"border-box",position:"relative",maxHeight:"90vh",overflowY:"auto",transform:dragY?`translateY(${dragY}px)`:"none",transition:dragY?"none":"transform 0.25s ease",touchAction:dragY>0?"none":"auto"}}>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"14px 16px",fontSize:15,marginBottom:18}}>
              ...
              {(()=>{const currentAtts=(getAttendees(selectedShow)||"").split(",").map(x=>x.trim()).filter(Boolean);const removeAtt=(name)=>{const next=currentAtts.filter(x=>x!==name).join(", ");saveAttendeeOverride(selectedShow,next);};const addAtt=(name)=>{if(!name||currentAtts.includes(name))return;saveAttendeeOverride(selectedShow,[...currentAtts,name].join(", "));};const availPeople=people.filter(p=>!currentAtts.includes(p));return(
              <div style={{gridColumn:"1 / -1"}}>
                <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:4,flexWrap:"wrap"}}>
                  ...
                  {attendeeOverrides[reviewKey(selectedShow)]!=null&&<span style={{fontSize:10,color:"#FB923C",fontWeight:600}}>edited</span>}
                </div>
                <div style={{display:"flex",gap:5,flexWrap:"wrap",alignItems:"center"}}>
                  {currentAtts.map((p,pi)=>(
                    <span key={pi} style={{display:"inline-flex",alignItems:"center",gap:4,background:"rgba(128,128,128,0.12)",color:TXT,padding:"4px 10px",borderRadius:20,fontSize:13,fontWeight:600}}><UserIcon/>{p}<span onClick={()=>removeAtt(p)} style={{cursor:"pointer",opacity:0.6,marginLeft:2,fontSize:12}}>✕</span></span>
                  ))}
                  <div style={{position:"relative",display:"inline-block"}}>
                    <button onClick={()=>{setAttDropOpen(!attDropOpen);setAttNewInput("");}} style={{display:"inline-flex",alignItems:"center",gap:3,padding:"4px 10px",borderRadius:20,border:`1px dashed ${CARD_BORDER}`,background:"transparent",color:TXT3,fontSize:12,fontWeight:600,cursor:"pointer"}}>+ Add</button>
                    {attDropOpen&&(
                      <div style={{position:"absolute",bottom:"calc(100% + 4px)",left:0,zIndex:60,background:"var(--card-solid)",border:`1px solid ${CARD_BORDER}`,borderRadius:12,padding:6,minWidth:180,maxHeight:220,overflowY:"auto",boxShadow:"0 10px 30px rgba(0,0,0,0.6)"}}>
                        {availPeople.map(p=>(
                          <div key={p} onClick={()=>{addAtt(p);setAttDropOpen(false);}} style={{padding:"7px 10px",cursor:"pointer",fontSize:13,color:TXT,borderRadius:8,display:"flex",alignItems:"center",gap:6}}><UserIcon/>{p}</div>
                        ))}
                        <div style={{padding:"6px 4px",borderTop:availPeople.length?`1px solid ${CARD_BORDER}`:"none",display:"flex",gap:4,alignItems:"center"}}>
                          <input value={attNewInput} onChange={e=>setAttNewInput(e.target.value)} placeholder="New person..." onKeyDown={e=>{if(e.key==="Enter"&&attNewInput.trim()){addAtt(attNewInput.trim());setAttNewInput("");setAttDropOpen(false);}}} style={{flex:1,padding:"5px 8px",borderRadius:8,border:`1px solid ${CARD_BORDER}`,background:"var(--card)",color:TXT,fontSize:12,outline:"none",minWidth:0}}/>
                          <button onClick={()=>{if(attNewInput.trim()){addAtt(attNewInput.trim());setAttNewInput("");setAttDropOpen(false);}}} style={{padding:"5px 8px",borderRadius:8,border:"none",background:ACCENT,color:"#fff",fontSize:11,fontWeight:700,cursor:"pointer",flexShrink:0}}>Add</button>
                        </div>
                      </div>
                    )}
                  </div>
            ...
            {selectedShow.booked===1&&<button onClick={()=>{if(window.confirm("Remove this booking?\n\n"+selectedShow.name+(selectedShow.date?" ("+selectedShow.date+")":"")))unbookShow(selectedShow);}} style={{width:"100%",marginTop:8,padding:"10px",borderRadius:10,border:`1px solid rgba(239,68,68,0.3)`,background:"rgba(239,68,68,0.08)",color:"#EF4444",fontSize:13,fontWeight:700,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",gap:6}}>Remove Booking</button>}
          </div>
        </div>
      )}
...
function ShowCard({show,onClick,review,onRate,wishlist,interest,onInterest,tags,onAddTag,onRemoveTag,attendees}){
      ...
      <div style={{flex:1,padding:"10px 14px",minWidth:0}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",gap:8}}>
          ...
          <div style={{textAlign:"right",flexShrink:0,maxWidth:"46%"}}>
            ...
            {(attendees||show.attendees)&&<div style={{display:"flex",gap:4,marginTop:8,flexWrap:"wrap",justifyContent:"flex-end"}}>{(attendees||show.attendees).split(",").map((p,pi)=>(<span key={pi} style={{display:"inline-flex",alignItems:"center",gap:3,background:"rgba(255,255,255,0.07)",color:TXT2,padding:"2px 8px",borderRadius:10,fontSize:13,fontWeight:600}}><UserIcon/>{p.trim()}</span>))}</div>}
          </div>
        </div>
      </div>
    </div>
...
function MultiDrop({open,onToggle,label,icon,options,selected,onSelect,onClear,accent="#93C5FD",up}){
  ...
  return (
    <div style={{position:"relative"}}>
      ...
      {open&&(<div style={{position:"absolute",...(up?{bottom:"calc(100% + 6px)"}:{top:"calc(100% + 6px)"}),right:0,zIndex:60,background:"var(--card-solid)",border:`1px solid ${CARD_BORDER}`,borderRadius:12,padding:6,minWidth:180,maxHeight:280,overflowY:"auto",boxShadow:"0 10px 30px rgba(0,0,0,0.6)"}}>
